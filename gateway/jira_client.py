...
class JiraClient:
    ...
    def create_issue(self, summary: str, description: str, issue_type: str) -> dict:
    ...
    def transition_to_in_progress(self, issue_key: str) -> None:
        ...
        target = next(
            (
                ...
                if t["name"].lower() == self.cfg.in_progress_status.lower()
            ),
            None,
        )
        if target is None:
            ...
            raise JiraError(
                f"no encuentro la transición '{self.cfg.in_progress_status}' "
